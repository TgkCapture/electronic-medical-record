...
public class view_Accountants implements ActionListener {
    ...
    public view_Accountants() {
        ...
        table_panel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "View Accountants",
        ...
        try {
            String query = "SELECT * FROM `accountants-register`";
    ...
    public void search() {
        ...
        try {
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/accounts_db", "root", "");
            String query = "SELECT `First-name`, `Last-name`, `Username`, `Email` FROM `accountants-register` WHERE `Username`  LIKE '%"
                    + value + "%' OR `ID` LIKE '%" + value + "%' ";
